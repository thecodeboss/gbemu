...
function hasReachedForeverLoop(
  ...
  const repeats =
    opcode === FOREVER_LOOP_OPCODE && operand === FOREVER_LOOP_OPERAND ? 1 : 0;
  ...
  try {
    ...
    while (performance.now() < deadline) {
      ...
      const signatureMatches = MEMORY_SIGNATURE.every(
        (byte, index) =>
          (emulator.bus.readByte(MEMORY_SIGNATURE_ADDRESS + index) & 0xff) ===
          byte,
      );
      ...
      runningSeen =
        runningSeen || (signatureSeen && status === MEMORY_RUNNING_VALUE);

      if (runningSeen && status !== MEMORY_RUNNING_VALUE) {
        if (status === 0 && !passSeen) {
          const textBytes = Array.from(
            { length: 32 },
            (_, idx) => emulator.bus.readByte(MEMORY_TEXT_ADDRESS + idx) & 0xff,
          );
          ...
          throw new Error(
            `ROM stopped with status 0 but without reporting pass; text="${truncateSerialOutput(textOut)}", serial="${truncateSerialOutput(serialLog)}", rawText=${textBytes
              ...
              .join(
                " ",
              )}, textPtr=0x${textPointerHi.toString(16).padStart(2, "0")}${textPointerLo.toString(16).padStart(2, "0")}, testCode=0x${testCode
              .toString(16)
              ...
              .padStart(
                2,
                "0",
              )}${testNameLo.toString(16).padStart(2, "0")}, pc=0x${pc.toString(16).padStart(4, "0")} (steps=${steps})`,
          );
        }
        if (status !== 0) {
          throw new Error(
