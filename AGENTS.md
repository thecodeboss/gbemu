...
- Supabase auth: `AuthProvider` (`src/hooks/use-auth.tsx`) wraps the app in `src/main.tsx` to expose `user/session/loading`; the `/login` route uses Supabase OAuth for Google or Discord via `supabase.auth.signInWithOAuth`. The client lives in `src/lib/supabase.ts` and expects `VITE_SUPABASE_URL` + `VITE_PUBLIC_SUPABASE_ANON_KEY` (publishable key) env vars, with sessions persisted/auto-refreshed. There is no Supabase CLI config checked in; supply the env vars directly when running locally.
