...
- Writing to `$FF46` now schedules a 160-byte OAM DMA over 640 + 4 T-cycles (one byte every 4 T-cycles with a 1 M-cycle tail), blocking CPU reads/writes to the OAM window with `0xff` while allowing other regions to respond normally. Instruction fetches account for per-byte timing so operand fetches near the DMA tail see the correct blocking/unblocking behavior.
