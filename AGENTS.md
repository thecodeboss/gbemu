...
- Emulator highlights:
  ...
  - Frame sequencer clocks length on steps 0/2/4/6, sweep on 2/6, and envelope on 7, synced to the DIV bit-4 phase (bit-13 when double-speed) when NR52 powers on or DIV is reset; triggering no longer drops channels that start with volume 0 if the envelope is configured to increase.
  ...
  - `Apu` emulates all four channels (square 1 with sweep, square 2, wave, noise) with length/envelope/sweep/LFSR, mixes at 48 kHz, high-pass-filters DC, and resamples to the host AudioContext sample rate.
...
- Shadcn UI components (Tailwind + Radix) provide most primitives; follow https://ui.shadcn.com/docs when touching `apps/web/src/components` so generated styles stay consistent. `components.json` now also registers the Supabase UI registry (`@supabase`).
...
- Supabase auth: `AuthProvider` (`src/hooks/use-auth.tsx`) wraps the app in `src/main.tsx` to expose `user/session/loading`; the `/login` route renders `LoginForm` (`src/components/login-form.tsx`) which kicks off Google OAuth via `supabase.auth.signInWithOAuth`. The client lives in `src/lib/supabase/client.ts` and expects `VITE_SUPABASE_URL` + `VITE_PUBLIC_SUPABASE_ANON_KEY` (publishable key) env vars, with sessions persisted/auto-refreshed. Supabase CLI config sits under `apps/web/supabase` (Google external provider enabled in `config.toml`, secrets pulled from `SUPABASE_AUTH_EXTERNAL_GOOGLE_CLIENT_SECRET`; `.gitignore` filters CLI/dotenv artifacts).
- `src/hooks/use-current-rom.tsx` holds the current ROM context; `MenuCard` writes to it on file/recent selection (and persists to recents) and the emulator hooks react to changes.
...
- `src/hooks/use-is-mobile-device.ts` replaces the former mobile helper; viewport height/media-query helpers live in `hooks/viewport.ts`.
- `src/app.tsx` derives `phase` directly (`menu`/`loading`/`running` based on ROM + loading state), logs errors, and renders without the old `ErrorCard`.
- The web UI is installable as a PWA: manifest + service worker live in `apps/web/public` and register via `registerServiceWorker` in `src/main.tsx`. Replace the placeholder icons in `apps/web/public/icons/` with branded assets and adjust `manifest.webmanifest` if names/colors change.
...
- ROMs now start running immediately after selection; the emulator provider handles load/start. Breakpoints/debug UI were removed from the main app.
