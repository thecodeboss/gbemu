...
- Game options live in `src/hooks/use-game-options.tsx` + `components/game-options-dialog.tsx`; an Options button on the home/emulator views opens the modal, and the speed multiplier (0.5x-5x via the `ButtonsRadio` component) forwards to `RuntimeClient#setSpeedMultiplier`.
